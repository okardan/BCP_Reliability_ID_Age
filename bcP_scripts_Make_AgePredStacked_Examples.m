...
% Make and save the SVR models for predicting age for 50*10 resamples for
% visualization purposes and future external cross-validation
% the script generates AgePredStacked_Examples.csv used in the other
% scripts
...
%            b = size(sub_data.GordonMatPA,2)
   ...
   xylabel = 'Age (months)';
    ...
    % if cvn234 == 4
    %     xylabel = 'Head Circumference (inches)';
    % end
        Y = all_ages;
        
        X = [atanh(all_braincondat_fullnet)];
        MdlLin1 = fitrsvm(X,Y,'Standardize',true,'BoxConstraint',Inf,'Epsilon',0.00001,'KFold',10);
%         MdlLin11 = MdlLin1; MdlLin11.X=[]; #need MATLAB Comp vision library to
%         save compact model (!)
        save(['Example_trained_Models\mdl_',num2str(zx),'.mat'],'MdlLin1');
...
T = table(Session,Dat,Age,Volumes,...
    Pred_Age_totFC,...
    FDs, aQC, fQC, TRs);
writetable(T,'Example_trained_Models\AgePredStacked_Examples.csv');